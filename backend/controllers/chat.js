...
const getMyGroups = TryCatch(async (req, res, next) => {
  ...
  const transformGroups = myGroups.map(
    ({ _id, creator, groupChat, name, members }) => {
      return {
        _id,
        name,
        groupChat,
...
const removeMembers = TryCatch(async (req, res, next) => {
  ...
  const isIncludes = chat.members.includes(userId);
  ...
  const retu = {
    ...
    members: chat.members.filter((member) => member.toString() != userID),
  };
